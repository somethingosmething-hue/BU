...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      try {
        ...
        if (Object.keys(editData).length > 0) {
          await interaction.guild.members.editMe(editData).catch(e => 
            console.error('[serverprofile] Failed to update member:', e.message)
          );
        }
      } catch (e) {
        console.error('[serverprofile] Error updating guild member:', e);
      }
      ...
      console.error('[serverprofile] error:', error);
      await interaction.reply({
