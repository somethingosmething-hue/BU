...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (sub === 'ban') {
      if (needsPerm('BanMembers')) return;
    ...
    if (sub === 'kick') {
      if (needsPerm('KickMembers')) return;
      const target = interaction.options.getUser('user');
      const reason = interaction.options.getString('reason') || 'No reason provided';
      const member = await interaction.guild.members.fetch(target.id).catch(() => null);
    ...
    if (sub === 'timeout') {
      if (needsPerm('ModerateMembers')) return;
      const target   = interaction.options.getUser('user');
    ...
    if (sub === 'purge') {
      if (needsPerm('ManageMessages')) return;
      ...
      const deleted = await interaction.channel.bulkDelete(toDelete, true).catch(e => { return { size: 0, error: e.message }; });
    ...
    if (sub === 'slowmode') {
      if (needsPerm('ManageChannels')) return;
      const durStr  = interaction.options.getString('duration');
    ...
    if (sub === 'nickname') {
      if (needsPerm('ManageNicknames')) return;
      const target   = interaction.options.getUser('user');
